...
class SearchFlightResults(BaseDriver):
    ...
    SEARCH_FLIGHTS_RESULTS = "//span[contains(text(),'Non Stop') or contains(text(),'1 Stop') or contains(text(),'2 Stop')]"
    ...
    def filter_flights_by_stop(self, by_stop):
        if by_stop == "1 Stop":
            ...
            self.log.warning("Please provide a valid filter option")           